...
import { FileText, Plus, Edit, Trash2, Eye, EyeOff, Search, Trophy, Database, X, Check, Filter, ChevronDown, Users, Clock, BarChart2 } from 'lucide-react'
...
export default function AdminProblemsPage() {
  ...
  const handleBatchAction = async (action: 'publish' | 'unpublish' | 'delete' | 'contest') => {
    if (selectedIds.size === 0) return
    if (action === 'delete' && !confirm(`确定要删除选中的 ${selectedIds.size} 个题目吗？此操作无法撤销。`)) return
      ...
      <div className="space-y-6">
            ...
            <div className="flex gap-2 flex-wrap">
              {selectMode ? (
                <button
                  ...
                  className="btn btn-ghost text-sm flex items-center gap-2 text-primary"
                >
                  <X className="w-4 h-4" />
                  取消选择
                </button>
                    ...
                    <div className="space-y-1">
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                          difficultyFilter === 'all' ? 'bg-primary/5 text-primary' : 'text-muted-foreground hover:bg-muted/50'
                        }`}
                      >
                        全部难度
                      </button>
                      {DIFFICULTIES.map(diff => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                            difficultyFilter === diff ? 'bg-primary/5 text-primary' : 'text-muted-foreground hover:bg-muted/50'
                          }`}
                        >
          ...
          {selectMode && (
            <div className="mt-4 flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20">
              <span className="text-sm text-primary font-medium">
                {selectedIds.size > 0 ? `已选择 ${selectedIds.size} 项` : '请选择题目'}
              </span>
              {selectedIds.size > 0 && (
                <>
                  ...
                  <button
                    ...
                    className="text-xs px-3 py-1.5 bg-secondary/10 text-secondary-light rounded-lg hover:bg-secondary/100/30 transition-colors disabled:opacity-50 flex items-center gap-1"
                  >
                    <Eye className="w-3 h-3" />
                    公开
                  </button>
                  <button
                    onClick={() => handleBatchAction('contest')}
                    disabled={isBatchProcessing}
                    className="text-xs px-3 py-1.5 bg-accent/10 text-accent-light rounded-lg hover:bg-accent/100/30 transition-colors disabled:opacity-50 flex items-center gap-1"
                  >
                  ...
                  <button
                    ...
                    disabled={isBatchProcessing}
                    className="text-xs px-3 py-1.5 bg-muted/50 text-muted-foreground rounded-lg hover:bg-muted/500/30 transition-colors disabled:opacity-50 flex items-center gap-1"
                  >
                  ...
                  <button
                    ...
                    disabled={isBatchProcessing}
                    className="text-xs px-3 py-1.5 bg-error/10 text-error rounded-lg hover:bg-error/100/30 transition-colors disabled:opacity-50 flex items-center gap-1"
                  >
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {selectMode && filteredProblems.length > 0 && (
              <div className="flex items-center gap-3 px-2">
                <button
                  onClick={selectAll}
                  className="text-xs text-muted-foreground hover:text-foreground transition-colors"
                >
                  {selectedIds.size === filteredProblems.length ? '取消全选' : '全选'}
                </button>
            ...
            {filteredProblems.map((problem) => (
              <div
                key={problem.id}
                onClick={() => selectMode ? toggleSelect(problem.id) : openProblemDetail(problem)}
                className={`card p-4 cursor-pointer transition-all hover:border-primary/30 ${
                  selectedIds.has(problem.id) ? 'ring-2 ring-primary/50 border-primary/50' : ''
                }`}
              >
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <h3 className="text-foreground font-medium truncate">{problem.title}</h3>
                          ...
                          {(problem.aiStatus === 'AI_GENERATED' || problem.aiStatus === 'GENERATED') && !problem.isVerified && (
                            <span className="tag tag-error text-[10px] px-1.5 py-0.5 flex items-center gap-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <span className={`tag text-[10px] px-2 py-0.5 rounded-full border ${getDifficultyBgClass(problem.difficulty)}`}>
                            {problem.difficulty}
                          </span>
                          
                          {problem.tags.slice(0, 3).map((tag, idx) => (
                            <span key={idx} className="text-[10px] text-muted-foreground bg-muted/50 px-2 py-0.5 rounded">
                              {tag}
                            </span>
                          ))}
                      ...
                      <div className="flex items-center gap-4 flex-shrink-0">
                        <div className="text-right">
                          <div className="text-xs text-muted-foreground">
                            提交 <span className="text-foreground font-medium">{problem.totalSubmit}</span>
                          </div>
                          <div className="text-xs text-muted-foreground">
                            通过 <span className="text-secondary-light font-medium">{problem.totalAccepted}</span>
                            {problem.totalSubmit > 0 && (
                              <span className="text-secondary-light ml-1">
                                ({((problem.totalAccepted / problem.totalSubmit) * 100).toFixed(0)}%)
                              </span>
                            )}
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                            problem.visibility === 'public' || (!problem.visibility && problem.isPublic)
                              ? 'bg-secondary/10 text-secondary-light hover:bg-secondary/100/30'
                              : problem.visibility === 'contest'
                              ? 'bg-accent/10 text-accent-light hover:bg-accent/100/30'
                              : 'bg-muted/50 text-muted-foreground hover:bg-muted/500/30'
                          }`}
                        >
                          {problem.visibility === 'public' || (!problem.visibility && problem.isPublic) ? (
                            <>
            ...
            {filteredProblems.length === 0 && (
              <div className="card p-12 text-center">
                ...
                <p className="text-muted-foreground">
                  {searchQuery || difficultyFilter !== 'all' || aiStatusFilter !== 'all'
                    ? '没有找到匹配的题目'
            ...
            <div className="p-4 overflow-y-auto flex-1 custom-scrollbar">
              ...
              {selectedProblem.description && (
                <div className="mb-6">
                  ...
                  <div className="bg-muted/50 rounded-lg p-4 text-sm text-muted-foreground whitespace-pre-wrap max-h-40 overflow-y-auto custom-scrollbar">
                    {selectedProblem.description}
                  </div>
                </div>
              )}
              ...
              {selectedProblem.input && (
                <div className="mb-4">
                  ...
                  <div className="bg-muted/50 rounded-lg p-4 text-sm text-muted-foreground whitespace-pre-wrap max-h-32 overflow-y-auto custom-scrollbar">
                    {selectedProblem.input}
                  </div>
                </div>
              )}
