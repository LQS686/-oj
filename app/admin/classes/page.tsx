      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredClasses.map((classData) => (
            <div key={classData.id} className="card p-6 group">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="text-lg font-bold text-foreground">{classData.name}</h3>
                    <span className={`tag ${classData.isPublic ? 'tag-success' : 'tag'}`}>
                      {classData.isPublic ? '公开' : '私有'}
                    </span>
                  </div>
                  <p className="text-sm text-muted-foreground line-clamp-2">{classData.description || '暂无描述'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <span>{classData.owner.username}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <Users className="w-4 h-4" />
                  <span>{classData._count?.members || 0} 成员</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground col-span-2">
                  <Calendar className="w-4 h-4" />
                  <span>{new Date(classData.createdAt).toLocaleDateString('zh-CN')}</span>
                </div>
              </div>
