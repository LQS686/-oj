...
function AdminClassesPageContent() {
  ...
  const [editClassId, setEditClassId] = useState<string | null>(() => {
  ...
  const filteredClasses = classes.filter((classData) => {
    ...
    return (
      classData.name.toLowerCase().includes(q) ||
      (classData.description || '').toLowerCase().includes(q)
    )
  })
    ...
    <>
      <AdminPageShell width="list" className="space-y-6">
        ...
        <DataTable<Class>
          ...
          onRowClick={(row) => setEditClassId(row.id)}
        />
      </AdminPageShell>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[110]">
          <div className="card p-6 max-w-md w-full mx-4">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={handleDeleteClass} className="btn btn-destructive">
                确认删除
              </button>
            </div>
          </div>
