  ...
  const columns: Column<AdminTraining>[] = [
    {
      ...
      render: (_, item) => (
        <div onClick={(e) => e.stopPropagation()}>
          ...
          {item.isRecommended && <span className="ml-2 text-xs text-warning">推荐</span>}
        </div>
      ),
    },
    {
      ...
      render: (_, item) => (
        <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
          <Link
            href={`/training/${item.id}`}
            target="_blank"
            className="btn-icon-sm hover:bg-muted text-muted-foreground hover:text-foreground"
            title="预览"
          >
          ...
          <button
            ...
            className="btn-icon-sm hover:bg-error/10 text-muted-foreground hover:text-error"
            title="删除"
          >
