...
export default function AssignmentDetailPage() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setObjectiveSubmissions(
          Array.isArray(data.data.objectiveSubmissions) ? data.data.objectiveSubmissions : []
        )
        setAllObjectiveSubmissions(
          Array.isArray(data.data.allObjectiveSubmissions) ? data.data.allObjectiveSubmissions : []
        )
      } else {
  ...
  const handleObjectiveSubmitted = useCallback((result: ObjectiveSubmittedResult | null) => {
    if (!result) return
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'active':
        return { label: '进行中', color: 'text-secondary bg-secondary/10 border-secondary/20' }
      case 'upcoming':
        return { label: '未开始', color: 'text-primary bg-primary/10 border-primary/20' }
      case 'ended':
        return { label: '已结束', color: 'text-muted-foreground bg-muted border-border' }
      default:
  ...
  const canViewCompletion =
    ...
    (() => {
      ...
      return (
        (me as { permissions?: { canViewStats?: boolean } } | undefined)?.permissions
          ?.canViewStats === true
      )
    })()
  ...
  const selectedObjectiveIndex = selectedIndex >= problemCount ? selectedIndex - problemCount : null
  ...
  const solvedCount =
    ...
    ).size + new Set(objectiveSubmissions.filter((s) => s.isCorrect).map((s) => s.questionId)).size
  ...
  const effectiveViewTab: ViewTab = viewTab === 'completion' && !isAdminOrOwner ? 'info' : viewTab

  const tabs = [
    { key: 'info' as const, label: '简介', icon: Info },
    { key: 'problems' as const, label: '题目', icon: FileText },
    ...(isAdminOrOwner ? [{ key: 'completion' as const, label: '完成情况', icon: BarChart3 }] : []),
  ]
    ...
    <div className="min-h-screen bg-background pb-20 lg:pb-6">
      <PageContainer variant="workspace" className="py-4">
        <EntityDetailHeader
          ...
          title={
            <>
              <h1 className="text-page-title text-foreground truncate">{assignment.title}</h1>
              <span
                            ...
                            {objectiveQuestions.map((q, idx) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <span
                                    className={`tag ${OBJECTIVE_QUESTION_TYPE_TAG_CLASSES[q.type]}`}
                                  >
            ...
            <ProblemWorkspaceShell
              ...
              rightPanel={
                <ProblemSubmitColumn
                  ...
                  problemId={
                    String(problemDetail?.id || problemDetail?._id || selectedProblem?.id || '') ||
                    null
                  }
                  statusBanner={
                    <>
                      {user && assignment.status === 'upcoming' && (
                        <div className="p-2.5 rounded-lg bg-info/10 border border-info/20 text-info text-xs flex items-center gap-2">
                          ...
                          {assignment.startTime
                            ? `将在 ${formatDateTime(assignment.startTime)} 开放提交`
                            : '暂不可提交'}
                        </div>
                      )}
