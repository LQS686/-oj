      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground">班级题库</h1>
              <p className="text-muted-foreground text-sm">班级专属题目列表</p>
            </div>
          </div>
        </div>
        ...
        {filteredProblems.length === 0 ? (
          <div className="card-static rounded-2xl p-16 text-center">
            ...
            <div className="text-muted-foreground">
              {searchQuery || selectedDifficulty !== '全部' ? '没有符合条件的题目' : '班级暂未添加题目'}
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            {filteredProblems.map((problem, index) => {
              ...
              return (
                <Link
                  key={problem.id}
                  href={`/problem/${problem.id}?classId=${params.id}`}
