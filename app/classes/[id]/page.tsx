  ...
  if (error || !classData) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <p className="text-foreground text-xl font-semibold mb-2">{error || '班级不存在'}</p>
        <Link href="/classes" className="btn btn-primary mt-4">返回班级列表</Link>
      </div>
    </div>
  )

  const isMember = !!classData.members.find((m: any) => m.userId === user?.id)
  const isAdmin = isMember && ['owner', 'assistant'].includes(classData.members.find((m: any) => m.userId === user?.id)?.role)
        ...
        <div className="bg-white dark:bg-card rounded-xl border border-border shadow-sm overflow-hidden mb-4">
          <div className="p-5 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3.5 min-w-0">
              {classData.avatar ? (
                <img src={classData.avatar} alt={classData.name} className="w-12 h-12 rounded-full object-cover ring-2 ring-primary/20 shrink-0" />
              ) : (
              ...
              <div className="min-w-0">
                <h1 className="text-lg font-bold text-foreground truncate">{classData.name}</h1>
              </div>
            </div>
            ...
            <div className="flex gap-2 shrink-0">
              ...
              {!isMember && (
                <button onClick={handleJoinClass} className="btn-primary btn text-sm px-4">
                  <UserPlus className="w-3.5 h-3.5" /> 加入
                </button>
              )}
              {isMember && classData.members.find((m: any) => m.userId === user?.id)?.role !== 'owner' && (
                <button onClick={handleLeaveClass} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border text-sm hover:bg-error/10 hover:text-error transition-colors">
