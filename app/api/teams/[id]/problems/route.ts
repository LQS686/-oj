/**
 * 团队题目管理 API
 * - GET /api/teams/[id]/problems - 获取团队题目列表
 * - POST /api/teams/[id]/problems - 添加题目到团队（从公共题库或创建新题目）
 */
...
export async function GET(
  ...
  try {
    ...
    console.error('获取团队题目列表失败:', error)
    return NextResponse.json(
      { success: false, error: '获取团队题目列表失败' },
      { status: 500 }
    )
  }
}
...
/**
 * POST /api/teams/[id]/problems - 添加题目到团队
 */
export async function POST(
  ...
  try {
    ...
    const latestProblem = await prisma.problem.findFirst({
      where: { problemNumber: { startsWith: 'T' } }, // T for Team problem? Or just use P?
      // Use P for consistency, but maybe T to distinguish?
      // Or just generate Pxxxx like global problems.
      // Let's use T prefix for team private problems to avoid collision easily?
    ...
    console.error('添加团队题目失败:', error)
    return NextResponse.json(
      { success: false, error: '添加团队题目失败' },
      { status: 500 }
    )
  }
}
