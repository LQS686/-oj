/**
 * 团队作业统计 API
 * - GET /api/teams/[id]/assignments/[assignmentId]/statistics - 获取作业详细统计数据
 */
...
export async function GET(
  ...
  try {
    ...
    if (!member) {
      return NextResponse.json(
        { success: false, error: '只有团队成员可以查看统计数据' },
        { status: 403 }
      )
    }
    ...
    const memberStats = members.map((member) => {
      ...
      return {
        ...
        // remark: member.remark,  // Prisma schema TeamMember doesn't seem to have remark? 
        // Let's check schema. TeamMember has no remark.
