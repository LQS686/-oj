...
// GET /api/teams/[id]/members/[memberId]/activity - 获取成员活动概况
export async function GET(
  ...
  try {
    ...
    if (!currentMember) {
      return NextResponse.json({ success: false, error: '您不是团队成员' }, { status: 403 })
    }
    ...
    // 统计数据
    const stats = {
      totalSubmissions: await prisma.teamAssignmentSubmission.count({
        where: {
          assignment: { teamId },
          userId: memberId
        }
      }),
      acCount: await prisma.teamAssignmentSubmission.count({
        where: {
          assignment: { teamId },
          userId: memberId,
          status: 'AC'
      ...
      totalNotes: await prisma.teamNote.count({
        where: {
          teamId,
          authorId: memberId
        }
