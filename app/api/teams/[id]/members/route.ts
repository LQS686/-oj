...
export async function GET(
  ...
  try {
    ...
    // 构建成员查询条件
    const memberWhere: any = { teamId: id }
    if (roleFilter) {
...
// POST /api/teams/[id]/members - 邀请成员加入团队
export async function POST(
  ...
  try {
    ...
    const isAdmin = inviter && (inviter.role === 'owner' || inviter.role === 'admin')
...
// DELETE /api/teams/[id]/members?userId=xxx - 移除成员
export async function DELETE(
  ...
  try {
    ...
    const operatorIsAdmin = operator && (operator.role === 'owner' || operator.role === 'admin')
...
// PATCH /api/teams/[id]/members - 更新成员角色
export async function PATCH(
  ...
  try {
    ...
    if (!isOwner) {
      return NextResponse.json(
        { success: false, error: '只有团队所有者可以设置管理员' },
        { status: 403 }
      )
    }
