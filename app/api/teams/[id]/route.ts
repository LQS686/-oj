...
export async function GET(
  ...
  try {
    ...
    const [memberCount, assignmentCount, noteCount] = await Promise.all([
      ...
      prisma.teamAssignment.count({ where: { teamId: id } }),
      prisma.teamNote.count({ where: { teamId: id } })
    ])
...
export async function DELETE(
  ...
  try {
    ...
    if (team.ownerId !== auth.userId) {
      return NextResponse.json(
        { success: false, error: '只有团队所有者可以解散团队' },
        { status: 403 }
      )
    }

    // 删除团队（Prisma 级联删除会处理关联数据，但为了保险，可以显式删除）
    // 注意：MongoDB 的 Prisma 级联删除需要正确配置 schema.prisma
    // schema.prisma 中 TeamMember, TeamAssignment 等都有 onDelete: Cascade
    // 所以直接删除 Team 即可
