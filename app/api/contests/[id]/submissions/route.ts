...
import { submitContestCode, listContestSubmissionsPaged } from '@/lib/contest/service'
...
export const GET = withApi.public(async (req, ctx) => {
  ...
  const bypassSeal =
    !!currentUser && (canAccessAdmin(currentUser) || access.contest?.authorId === currentUser.id)

  const result = await listContestSubmissionsPaged(contestId!, {
    page: toInt(q.page, 'page', 1),
    limit: toInt(q.limit, 'limit', 20),
    userId: bypassSeal || (q.userId && currentUser?.id === q.userId) ? q.userId : undefined,
