/**
 * 班级邀请码管理 API
 * - GET /api/classes/[id]/invites - 获取邀请码列表
...
export async function POST(
  ...
  try {
    ...
    const isAdmin = member && (member.role === 'owner' || member.role === 'assistant')
    ...
    let exists = await prisma.classInvite.findUnique({ where: { code } })
    
    // 如果邀请码已存在，重新生成
    while (exists) {
      code = generateInviteCode()
      exists = await prisma.classInvite.findUnique({ where: { code } })
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        maxUses: invite.maxUses,
        expiresAt: invite.expiresAt,
        inviteLink: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/classes/join?code=${code}`
      },
      message: '邀请码创建成功'
    })
  } catch (error: any) {
