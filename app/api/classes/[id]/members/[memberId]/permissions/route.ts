/**
 * 成员权限管理 API
 * - PATCH /api/classes/[id]/members/[memberId]/permissions - 更新成员权限
 */
...
export async function PATCH(
  ...
  try {
    ...
    const isOperatorAdmin = operator && (operator.role === 'owner' || operator.role === 'assistant')
    ...
    if (targetMember.role === 'owner') {
      return NextResponse.json(
        { success: false, error: '不能修改班级创建人的权限' },
        { status: 400 }
      )
    }

    // 只有所有者可以修改管理员的权限
    if (targetMember.role === 'assistant' && operator.role !== 'owner') {
      return NextResponse.json(
        { success: false, error: '只有班级创建人可以修改管理员的权限' },
        { status: 403 }
      )
    }
