...
export const GET = withApi.classRole(['owner', 'assistant', 'student'], async (req, ctx) => {
  const { id } = ctx.params
...
export const POST = withApi.classRole(
  ...
  async (req, ctx, { user, membership }) => {
    ...
    if (body.startTime && new Date(body.startTime).getTime() >= new Date(body.endTime!).getTime()) {
