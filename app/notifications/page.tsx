    ...
    },
    {
      ...
      render: (value, row) => (
        <div className="flex items-center gap-2 min-w-0">
          {!row.isRead && <span className="w-2 h-2 rounded-full bg-primary flex-shrink-0" />}
          <div className="min-w-0">
            <div
              className={`font-semibold truncate ${!row.isRead ? 'text-foreground' : 'text-muted-foreground'}`}
            >
              {value as string}
            </div>
            <p
              className={`text-sm truncate ${!row.isRead ? 'text-foreground/80' : 'text-muted-foreground'}`}
            >
              {row.content}
            </p>
          </div>
          ...
          <DataTable
            ...
            mobileCardRenderer={(row) => (
              <div className="space-y-3">
                ...
                <div>
                  <p
                    className={`text-label ${row.isRead ? 'text-muted-foreground' : 'text-foreground'}`}
                  >
                    {row.title}
                  </p>
                  {row.content && (
