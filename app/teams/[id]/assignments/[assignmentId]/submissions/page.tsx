...
export default function AssignmentSubmissionsPage({ params }: { params: Promise<{ id: string; assignmentId: string }> }) {
  const { id: teamId, assignmentId } = use(params)
  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        ...
        const response = await fetchWithAuth(
          `/api/teams/${teamId}/assignments/${assignmentId}/submissions?${params}`
        )
        const data = await response.json()
    ...
    fetchSubmissions()
  }, [teamId, assignmentId, filterUserId, filterProblemId, filterStatus, isFromLeaderboard])
      ...
      <div className="container mx-auto px-4 py-8">
        <button
          onClick={() => {
            ...
            router.push(`/teams/${teamId}/assignments/${assignmentId}?tab=${returnTab}`)
          }}
