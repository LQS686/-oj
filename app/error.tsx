  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="text-center max-w-md animate-fadeIn">
        ...
        <h1 className="text-status-title text-foreground mb-4">出错了</h1>
        <p className="text-muted-foreground text-lg mb-8">
          抱歉，系统遇到了一些问题。我们已经记录了此错误，请稍后重试。
        </p>

        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button onClick={reset} className="btn btn-primary inline-flex items-center gap-2">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mt-8 p-4 rounded-xl bg-error/10 border border-error/20 text-left">
            <p className="text-label text-error mb-2">错误详情：</p>
            <p className="text-xs text-error/80 font-mono break-all">{error.message}</p>
          </div>
        )}
      </div>
    </div>
