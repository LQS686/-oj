...
export default function RankPage() {
  ...
  const getRankIcon = (rank: number) => {
    if (rank === 1) {
      return (
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-accent to-accent-dark flex items-center justify-center shadow-lg shadow-accent/30">
    ...
    if (rank === 2) {
      return (
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-secondary to-secondary-dark flex items-center justify-center shadow-lg shadow-secondary/30">
    ...
    if (rank === 3) {
      return (
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-primary-dark flex items-center justify-center shadow-lg shadow-primary/30">
          <Medal className="w-4 h-4 text-white" />
        </div>
      )
    }
      ...
      <div className="container mx-auto px-4 py-8 flex-1 flex flex-col">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-foreground">全站排行榜</h1>
              <p className="text-muted-foreground text-sm mt-0.5">每 30 秒更新一次</p>
            </div>
          </div>
          <button 
            onClick={() => { setIsRefreshing(true); fetchRankings(1, activeTab, true); }}
        ...
        <div className="flex items-center gap-2 glass p-1.5 rounded-xl mb-6 w-fit">
          {[
                ...
                <div className="flex-1 flex items-center gap-3 min-w-0">
                  <Link href={`/profile/${user.id}`} className="flex items-center gap-3 flex-1 min-w-0 group">
                    <div className="relative">
                      ...
                      {isTop3 && (
                        <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-gradient-to-br from-accent to-accent-dark flex items-center justify-center">
      ...
      <AnimatePresence>
        {myRank && myRank.rank > 100 && (
          <motion.div
            ...
            animate={{ y: 0 }}
            exit={{ y: 100 }}
            className="fixed bottom-0 left-0 right-0 glass-strong border-t border-border z-50"
          >
