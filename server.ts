...
import { readBodyWithLimit, parseMultipart, extractMultipartBoundary } from './lib/http/multipart'
 ...
 */
async function handleAvatarChunkDirect(
  req: IncomingMessage,
  res: ServerResponse
): Promise<boolean> {
  ...
  logger.info('[chunk-direct] enter', {
  ...
  logger.info('[chunk-direct] parsed parts', {
    count: parts.length,
  ...
  if (isNaN(chunkIndex) || chunkIndex < 0 || chunkIndex > MAX_CHUNK_INDEX) {
    writeJson(res, 400, {
      ...
      error: `chunkIndex 超出范围 (0-${MAX_CHUNK_INDEX})`,
    })
    return true
  }
  ...
  if (chunkBuffer.length > MAX_CHUNK_SIZE) {
    writeJson(res, 400, {
      ...
      error: `分片大小超过限制 (Max ${MAX_CHUNK_SIZE} bytes)`,
    })
    return true
  }
...
async function getUserFromRawRequest(
  ...
  try {
    const payload = jwt.verify(token, process.env.JWT_SECRET!) as
      { userId?: string; tokenVersion?: number } | string
