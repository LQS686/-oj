...
说明：
- **系统设置**：站点级配置（AI 提供商、系统参数等），仅 `SYSTEM_ADMIN`。
...
| `withApi.classRole` | 班级角色（owner/assistant/student）       | 班级内部操作                   |
...
| 班级角色（API 值） | 班级内含义 | 数据库存储值 |
| ----------------- | ---------- | ------------ |
| `owner`           | 班主任     | `owner`      |
| `assistant`       | 助教       | `admin`      |
| `student`         | 学生       | `member`     |

- 班级角色定义见 [lib/class/roles.ts](../lib/class/roles.ts)：API/前端统一使用 `owner` / `assistant` / `student`，数据库存储为 `owner` / `admin` / `member`。
