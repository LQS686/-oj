...
interface ToolbarAction {
  icon: LucideIcon
  title: string
  run: (params: { selection: string; lineStart: string; hasSelection: boolean }) => {
    before: string
    after?: string
    placeholder?: string
    mode: InsertMode
...
function countWords(text: string): number {
  ...
  const latinWords = stripped.split(/[^A-Za-z0-9_]+/).filter((w) => w.length > 0)
        ...
        <div className="flex items-center gap-2">
          <span
            className={cn(
              ...
              submitHint.tone === 'success' && 'border-secondary/40 text-secondary bg-secondary/10',
