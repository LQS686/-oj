...
export default function SubmissionResultModal({
  ...
  return (
    ...
    >
      {isAC && isOpen && !isJudging && result?.submissionId && (
        <Confetti trigger={result.submissionId} />
      )}

      <div
        className="card-static rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden shadow-2xl transition-all duration-300"
              ...
              {judgeProgress && judgeProgress.totalTests > 0 && (
                <div className="mt-4 w-full max-w-xs mx-auto">
                  <div className="relative w-full h-2 bg-muted rounded-full overflow-hidden">
                    <div
                      className="absolute top-0 left-0 h-full bg-gradient-to-r from-primary to-primary-light transition-all duration-500 rounded-full"
                      style={{
