...
type PermissionReason =
  'ADMIN' | 'TEACHER' | 'ENOUGH_SCORE' | 'ASSIGNMENT_CONTEXT' | 'NO_SUBMISSION' | 'LOW_SCORE'
...
> = {
  ...
  NO_SUBMISSION: (result) => `请先尝试提交本题，达到 ${result.requiredScore} 分即可查看`,
  LOW_SCORE: (result) =>
    `再接再厉！当前最高分 ${result.bestScore ?? 0}，达到 ${result.requiredScore} 分即可查看`,
}

function getLockMessage(result: PermissionResult): string {
  return REASON_TEXT[result.reason as keyof typeof REASON_TEXT]?.(result) ?? '当前不可查看题解'
}
...
export default function SolutionTabPanel({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const response = await fetchWithCookie(`/api/solutions/${expandedId}${qs ? `?${qs}` : ''}`)
        const data = await response.json().catch(() => null)
        if (cancelled) return
  ...
  if (permissionLoading) {
    return (
      <div className="space-y-4" aria-busy="true" aria-live="polite">
        {[0, 1, 2].map((i) => (
          <div key={i} className="card-static p-5 flex gap-4 animate-pulse">
  ...
  if (permission && !permission.allowed) {
    return (
      <ListEmptyState icon={Lock} title="🔒 题解已锁定" description={getLockMessage(permission)} />
    )
  }
      ...
      {!solutionsLoading &&
        ...
        filteredSolutions.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground">暂无 {getLanguageLabel(languageFilter)} 题解</p>
            <button
              onClick={() => setLanguageFilter('all')}
