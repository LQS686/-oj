...
const CATEGORY_STYLE: Record<
  string,
  { bar: string; badge: string; icon: typeof BookOpen; label: string }
> = {
  official: {
    bar: 'bg-primary',
...
export function TrainingCard({ training, variant = 'default' }: TrainingCardProps) {
  ...
  if (variant === 'grid') {
    return (
      <Link href={`/training/${training.id}`} className={listGridCardLinkClass()}>
        <div className={LIST_GRID_CARD_META_ROW}>
          {cat && CatIcon ? (
            <span
              className={`inline-flex items-center gap-0.5 text-xs px-1.5 py-0.5 rounded ${cat.badge}`}
            >
        ...
        <div className={LIST_GRID_CARD_MIDDLE}>
          <div className="flex items-start gap-1 mb-1.5">
            ...
            <h3 className={LIST_GRID_CARD_TITLE}>{training.title}</h3>
          </div>
          {training.author && (
  ...
  return (
    ...
    >
      ...
      <div className="col-span-5 flex items-center gap-2 min-w-0">
        ...
        {cat && CatIcon && (
          <span
            className={`inline-flex items-center gap-0.5 text-xs px-1.5 py-0.5 rounded ${cat.badge} flex-shrink-0`}
          >
            <CatIcon className="w-3 h-3" />
            {cat.label}
          </span>
        )}
        <h3 className="text-subsection-title text-foreground group-hover:text-primary-light transition-colors truncate">
          {training.title}
        </h3>
      ...
      <div className="col-span-2 flex items-center gap-1.5 text-xs text-muted-foreground min-w-0">
        {training.author && (
          <>
            ...
            <span className="text-primary-light truncate">
              {training.author.nickname || training.author.username}
            </span>
          </>
        )}
      ...
      <div className="col-span-2 flex items-center justify-end gap-3 text-xs text-muted-foreground">
        ...
        <span className="text-muted-foreground/70">{training.problemCount} 题</span>
      </div>
    </Link>
  )
}
