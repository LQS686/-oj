...
export default function TrainingProblemMainHeader() {
  const params = useParams()
  const problemId = params.problemId as string
  const { trainingProblems } = useTrainingProblemWorkspace()
  ...
  if (!problem) {
    return <div className="mb-4 h-16 rounded-lg bg-muted/40 animate-pulse" aria-hidden />
  }

  const meta = trainingProblems.find((p) => p.id === problemId)
  const label =
    meta?.label || LETTERS[trainingProblems.findIndex((p) => p.id === problemId)] || ''
