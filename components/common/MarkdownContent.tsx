...
// 服务端进程级渲染缓存（LRU）：题面是「读多写少、渲染结果确定」的典型场景，
// 同一份 markdown 的服务端渲染结果（ReactNode）可跨请求复用，避免每个请求
// 都重跑 remark/rehype/KaTeX 全管线（大题面单次可达数百 ms）。
// 只存内存（不写 Redis，ReactNode 非 JSON 安全）；多实例各自缓存，首次各渲染一次。
const renderCache = new Map<string, ReactNode>()
const RENDER_CACHE_MAX = 512

function renderMarkdown(normalized: string): ReactNode {
