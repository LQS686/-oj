...
const statusConfig: Record<string, { label: string; className: string; iconColor: string }> = {
  ...
  PENDING: {
    label: '等待评测',
    className: 'text-primary-light bg-primary/10',
                ...
                >
                  <div className="flex items-center justify-between mb-0.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        className={`text-xs font-medium ${config?.iconColor || 'text-muted-foreground'}`}
                      >
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <div className="px-4 py-2.5 border-b border-border flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-subsection-title text-foreground">代码</span>
              {currentLang && (
            ...
            {currentCode && (
              <button
                onClick={handleCopy}
                className={`flex items-center gap-1 btn btn-sm transition-all ${
                  copied
...
export default function StudentCompletionTable({
  ...
  const getStatusDisplay = (submission?: StudentSubmission) => {
    ...
    const scoreClass = isAcceptedStatus(submission.status)
      ? 'text-secondary'
      : submission.score > 0
        ? 'text-accent'
        : 'text-muted-foreground'
    ...
    if (
      ...
      typeof submission.timeElapsedMs === 'number' &&
      submission.timeElapsedMs > 0
    ) {
      return (
        <div className="flex flex-col items-center gap-0.5 min-w-[4.5rem] cursor-pointer hover:opacity-90 transition-opacity">
          ...
          <span
            className={`text-xs font-medium flex items-center gap-0.5 ${cfg?.iconColor || 'text-muted-foreground'}`}
          >
    ...
    return (
      <div className="flex flex-col items-center gap-0.5 min-w-[4.5rem] cursor-pointer hover:opacity-90 transition-opacity">
        ...
        <span className={`text-xs font-medium ${cfg?.iconColor || 'text-muted-foreground'}`}>
          {statusLabel}
        </span>
        <span className="text-xs text-muted-foreground tabular-nums leading-tight">
          {formatDateTimeShort(submission.submittedAt ?? '')}
    ...
    <>
      <div className="bg-card rounded-lg border border-border overflow-hidden">
        <div className="px-5 py-3 border-b border-border flex items-center justify-between gap-3">
          <div>
            <h2 className="text-section-title text-foreground">学生作业完成情况</h2>
          ...
          <table className="table w-full text-sm">
            <thead>
              <tr className="bg-muted text-left border-b border-border">
                <th className="sticky left-0 bg-muted z-[1] w-[110px]">学生</th>
                {problems.map((problem, index) => (
                  <th
                    ...
                    className={`px-2 py-2 font-mono font-bold text-sm text-center min-w-[5.25rem] select-none text-foreground/60 ${
                      onProblemClick
                        ? 'cursor-pointer hover:text-primary-light transition-colors'
                        : ''
                    }`}
                    title={problem.title}
                  >
                ...
                >
                  <span className="inline-flex items-center justify-center w-full">
                    ...
                    {sortField === 'score' && (
                      <ChevronDown
                        className={`w-3 h-3 ml-1 transition-transform ${sortOrder === 'desc' ? '' : 'rotate-180'}`}
                      />
                    )}
                  </span>
                </th>
                <th className="text-center w-[72px]">完成数</th>
                <th
              ...
              {sortedStudents.length > 0 ? (
                sortedStudents.map((student) => (
                  <tr key={student.id} className="hover:bg-muted/[0.03] transition-colors">
                    <td className="sticky left-0 bg-background z-[1]">
                    ...
                    {objectiveList.map((question) => {
                      ...
                      return (
                        <td key={question.id} className="text-center align-middle">
                          {sub ? (
                            sub.isCorrect ? (
                    ...
                    <td className="text-center tabular-nums font-semibold text-foreground align-middle">
                      <span className="inline-flex items-center justify-center w-full">
                        {student.totalScore}
                      </span>
                    </td>
                    <td className="text-center tabular-nums text-muted-foreground align-middle">
                      <span className="inline-flex items-center justify-center w-full">
                        {student.completedCount} / {problems.length}
                      </span>
                    </td>
                    <td className="text-center tabular-nums text-muted-foreground align-middle">
                      <span className="inline-flex items-center justify-center w-full">
                ...
                <tr>
                  <td
                    colSpan={problems.length + objectiveList.length + 4}
          ...
          <div className="px-5 py-2.5 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
            ...
            <div className="flex items-center gap-4">
              <span>
                平均分：
                {Math.round(
                  filteredStudents.reduce((sum, s) => sum + s.totalScore, 0) /
                    filteredStudents.length || 0
                )}
              </span>
              <span>
                ...
                {formatDurationMs(
                  Math.round(
                    filteredStudents.reduce((sum, s) => sum + s.totalTimeMs, 0) /
                      filteredStudents.length || 0
                  )
                )}
              </span>
