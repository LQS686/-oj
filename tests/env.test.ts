...
describe('env', () => {
  ...
  it('validateEnvironment 在 HTTPS 生产站禁止 FORCE_SECURE_COOKIE=false', async () => {
    reloadEnv()
    vi.stubEnv('JWT_SECRET', 'a'.repeat(48))
    vi.stubEnv('DATABASE_URL', 'mongodb://localhost:27017/test?replicaSet=rs0')
    vi.stubEnv('NODE_ENV', 'production')
    vi.stubEnv('FRONTEND_URL', 'https://dsoj.run')
    vi.stubEnv('REDIS_URL', 'redis://localhost:6379')
    vi.stubEnv('ENCRYPTION_KEY', 'a'.repeat(32))
    vi.stubEnv('FORCE_SECURE_COOKIE', 'false')
    const { validateEnvironment } = await import('../lib/env')
    expect(() => validateEnvironment()).toThrow(/FORCE_SECURE_COOKIE/)
  })

  it('validateEnvironment 在 HTTP 生产站允许 FORCE_SECURE_COOKIE=false', async () => {
    reloadEnv()
    vi.stubEnv('JWT_SECRET', 'a'.repeat(48))
    vi.stubEnv('DATABASE_URL', 'mongodb://localhost:27017/test?replicaSet=rs0')
    vi.stubEnv('NODE_ENV', 'production')
    vi.stubEnv('FRONTEND_URL', 'http://127.0.0.1')
    vi.stubEnv('REDIS_URL', 'redis://localhost:6379')
    vi.stubEnv('ENCRYPTION_KEY', 'a'.repeat(32))
    vi.stubEnv('FORCE_SECURE_COOKIE', 'false')
    const { validateEnvironment } = await import('../lib/env')
