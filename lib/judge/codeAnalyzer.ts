...
function estimateComplexity(
  ...
  const nestedLoops =
    (code.match(/for\s*\(/g) || []).length + (code.match(/while\s*\(/g) || []).length
  const nestedIfs = (code.match(/if\s*\(/g) || []).length
  const functionCount = (
    code.match(/function\s+\w+|def\s+\w+|int\s+\w+\s*\(|void\s+\w+\s*\(/g) || []
  ).length
...
function analyzeCode(code: string, language: string): CodeAnalysisResult {
  ...
  if (langKey && DANGEROUS_PATTERNS[langKey as keyof typeof DANGEROUS_PATTERNS]) {
    // 安全说明：以下正则仅做静态告警，真正的安全边界是评测沙箱（runner.sh + ulimit 资源限制）。
    ...
    patterns.forEach(({ pattern, name }) => {
      ...
      if (regex.test(code)) {
        result.warnings.push(
          `检测到可能受限的模式: ${name}（最终是否放行由 runner.sh 资源限制决定）`
        )
