...
export async function incrementProblemSubmitCount(problemId: string) {
  ...
  await db
    .collection('Problem')
    .updateOne({ _id: new ObjectId(problemId) }, { $inc: { totalSubmit: 1 } })
}
...
export async function updateSubmissionDirect(
  ...
  return withRetry(
    async () => {
      ...
      const result = await db.collection('Submission').updateOne(filter, { $set: sanitized })
      return {
...
export async function incrementProblemAcceptedCount(problemId: string) {
  ...
  await db
    .collection('Problem')
    .updateOne({ _id: new ObjectId(problemId) }, { $inc: { totalAccepted: 1 } })
}
 ...
 */
export async function isFirstAccepted(
  problemId: string,
  userId: string,
